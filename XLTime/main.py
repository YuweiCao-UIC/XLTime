...
def main():
    ...
    if args.backbone == 'XLMR':
        ...
        model = XLTime_XLMR(model_type=model_type, n_labels=num_labels, hidden_size=hidden_size,
                                        dropout_p=args.dropout, device=device)
    elif args.backbone == 'mBERT':                                   
        model = XLTime_mBERT(n_labels=num_labels, hidden_size=hidden_size,
                                        dropout_p=args.dropout, device=device)
    else:
    ...
    if args.max_seq_length == 0:
        ...
        #max_seq_length_bc = findout_max_seq_length(train_examples_bc + val_examples_bc + test_examples_bc, model.encode_word)
